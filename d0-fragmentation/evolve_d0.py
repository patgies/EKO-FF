...
import parametrizations as ph  # noqa: E402
...
def evolve_bcfy(q_values, zgrid=None, n_integration_cores=4, n_xgrid=100):
    ...
    D_D0(z, Q) = 0.168 * D_P_evolved(z, Q)
               + 0.39 * theta(mD/mDstar - z) * (mDstar/mD)
                      * D_V_evolved(z * mDstar/mD, Q)

    P and V are evolved as two independent time-like DGLAP sets (both from
    mu0=mc) and combined after evolution. The (mDstar/mD) factor is the
    Jacobian of the z -> z*mDstar/mD rescaling, same as in parametrizations.bcfy_d0
    and the QCDNUM version.
    ...
    out = {
        q: 0.168 * dp[q] + np.where(step, 0.39 * ph.MASS_RATIO * dv[q], 0.0)
        for q in dp
    }
    return zgrid, out
